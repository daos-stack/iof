...
orterun --tag-output -np 4 $CMD_PREFIX test_ps --name service_set --is_service 1 : \
	-np 4 $CMD_PREFIX test_ps --name client_set --is_service 0 --attach_to service_set

orterun --tag-output -np 1 $CMD_PREFIX test_ps --name a --is_service 1 : \
...
orterun --tag-output -np 4 $CMD_PREFIX test_ps --name test_srv_set --is_service 1
